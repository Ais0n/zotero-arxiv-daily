...
| DEEPSEEK_API_KEY | API Key for DeepSeek-compatible LLM calls. | sk-xxx |
| DEEPSEEK_API_BASE | API base URL for DeepSeek-compatible LLM calls. | https://api.deepseek.com |

Optional repository variables:

| Key |Description | Example |
| :---  | :---  | :--- |
| MODEL | DeepSeek model used for summaries and affiliation extraction. Defaults to `deepseek-v4-flash`. | deepseek-v4-pro |
...
llm:
  api:
    ...
    base_url: ${oc.env:DEEPSEEK_API_BASE}
  generation_kwargs:
    model: ${oc.env:MODEL,deepseek-v4-flash}
  language: Chinese
...
source:
  arxiv:
    ...
    date: null # Optional exact date for local backfill reports, e.g. 2026-04-24.
...
source:
  arxiv:
    ...
    date: null # Optional exact arXiv submitted date for local backfill reports. Example: 2026-04-24
    max_results: 500 # Maximum arXiv API results to fetch when source.arxiv.date is set.
  biorxiv:
...
llm:
  ...
  # Arguments for the LLM API. See [here](https://platform.openai.com/docs/api-reference/chat/create) for more details.
    max_tokens: 16384
    model: ${oc.env:MODEL,deepseek-v4-flash}
  language: Chinese # Preferred language for the detailed paper summary. Example: Chinese
...
To compose a report for a previous arXiv date, pass a Hydra override:
```bash
uv run main.py source.arxiv.date=2026-04-24
```

This fetches arXiv papers submitted on that exact date for your configured categories, reranks them against your Zotero corpus, generates Chinese detailed summaries with the configured DeepSeek model, and sends the email. Add `executor.send_empty=true` if you want an email even when no papers are found.

To test only your SMTP settings without retrieving papers:
```bash
uv run main.py executor.test_email=true
```
